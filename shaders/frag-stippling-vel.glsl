...
void main() {
    ...
    if ((gl_FragCoord.x - 0.5) + (gl_FragCoord.y - 0.5) * width < u_nActiveParticles) {
        ...
        for (float i = 0.0; i < nParticles; i++) {
            ...
            vec3 bgColor = texture2D(u_bgTexture, (particlePosition.xy + u_textureOffset) / (2.0 * u_textureOffset)).rgb;
            float intensity = length(bgColor);
            float forceScalingFactor = 0.00002 * (1.0 + intensity);

            // Add the particle repulsion force
            float distanceDumping = 1.0 - smoothstep(intensity + 0.05, intensity + 0.1, distance);
