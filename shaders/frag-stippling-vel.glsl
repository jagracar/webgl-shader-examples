...
float get_particle_charge_range(vec3 position) {
	...
	return max(0.05, 0.02 * pow(dot(bgColor, vec3(1.0)), 3.0));
}
...
void main() {
	...
	if ((gl_FragCoord.x - 0.5) + (gl_FragCoord.y - 0.5) * width < u_nActiveParticles) {
		// Get the particle background color
		float intensity = get_particle_charge_range(position);
		...
		for (float i = 0.0; i < nParticles; i++) {
			...
			float particleIntensity = get_particle_charge_range(particlePosition);
			float forceScalingFactor = 0.0001;

			// Add the particle repulsion force
			float distanceDumping = 1.0 - step(intensity + particleIntensity, distance);
